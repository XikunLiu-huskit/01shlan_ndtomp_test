...
bool FrontEnd::InitWithConfig() {
    ...
    InitCluster(cluster_ptr_, config_node);
...
bool FrontEnd::InitCluster(std::shared_ptr<ClusterInterface>& cluster_ptr, const YAML::Node& config_node) {
    std::string cluster_method = config_node["cluster_method"].as<std::string>();
    LOG(INFO) << "Point Cloud Cluster Method: " << cluster_method;
    if (cluster_method == "Euclidean_Cluster") {
        cluster_ptr = std::make_shared<EuclideanCluster>(config_node[cluster_method]);
    }
    else {
        LOG(ERROR) << "Point cloud cluster method " << cluster_method << " NOT FOUND!";
        return false;
    }
...
bool FrontEnd::InitRegistration(std::shared_ptr<RegistrationInterface>& registration_ptr, const YAML::Node& config_node) {
    ...
    else if (registration_method == "NDT_DK") {
        registration_ptr = std::make_shared<NDTDKRegistration>(config_node[registration_method]);
    }

    else if (registration_method == "GICP_DK") {
        registration_ptr = std::make_shared<GICPDKRegistration>(config_node[registration_method]);
    }
...
bool FrontEnd::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose) {
    ...
    CloudData::CLOUD_PTR clustered_ptr(new CloudData::CLOUD());
    cluster_ptr_->Cluster(filtered_cloud_ptr, clustered_ptr);
    ...
    // 不是第一帧，就正常匹配
    registration_ptr_->ScanMatch(clustered_ptr, predict_pose, result_cloud_ptr_, current_frame_.pose);
    auto t2 = ros::WallTime::now();
    LOG(INFO) << "time:" << (t2 - t1).toSec() * 1000 << "[msec]" << std::endl;
    cloud_pose = current_frame_.pose;
